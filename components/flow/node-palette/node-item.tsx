...
export function NodeItem({ definition }: NodeItemProps) {
  ...
  return (
    <div
      ...
      className="flex items-center gap-1.5 rounded-lg select-none cursor-grab active:cursor-grabbing transition-colors hover:bg-white/6 bg-[#1e1f22] border border-[#2e3033] px-2.5 py-2"
      title={definition.label}
    >
      <DynamicIcon name={definition.iconName} color={definition.color} />
      <span
        className="flex-1 truncate text-sm text-[#c8cace]"
      >
        {definition.label}
      </span>
      {definition.isPro && (
